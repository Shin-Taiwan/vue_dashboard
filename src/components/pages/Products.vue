        ...
        <table class="table mt-4">
            ...
            <tbody>
                <tr v-for="(item) in products" :key="item.id">
                    ...
                    <td>
                    <button class="btn btn-outline-primary btn-sm" @click="openModel(false,item,false)">編集</button>
                    <button class="btn btn-outline-danger btn-sm" @click="openModel(false,item,true)">刪除</button>
                    </td>                   
                </tr>
            </tbody>
        </table>
...
export default{
    ...
    methods:{
        ...
        updateProduct(){
            ...
            let httpdMethod ='post';
            const vm = this;
            if(!vm.isNew){
                api = `${process.env.APIPATH}/api/${process.env.CUSTOMPATH}/admin/product/${vm.tempProduct.id}`;
                httpdMethod='put';
            }
            this.$http[httpdMethod](api,{data:vm.tempProduct}).then((response) => {
                console.log(response.data)
